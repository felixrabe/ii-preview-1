...
class DevBarDragHandle extends React.Component {
  render() {
    return (
      <div className='idev-devbar-drag-handle' style={style.DevBarDragHandle}>
        -- {this.props.children} --
      </div>
    )
  }
...
export default class DevBarView extends React.Component {
  ...
  componentWillUpdate(nextProps, nextState) {
    ...
    if (selfPath !== undefined) {
      ...
      this.context.dataAccess.set(selfPath, newConfig)
    }
  }
  ...
  render() {
    return (
      <div style={{height: '100%', position: 'relative'}}>
        ...
        <Draggable handle='.idev-devbar-drag-handle'>
          <DevBar view={this} />
        </Draggable>
      </div>
    )
