...
export default class App extends React.Component {
  constructor(props) {
    ...
    this.cmdLine = <CommandLine onReturn={this.cmdRunner.run} />

    this.state = {
      appClassNames: {'animate-none': true},
      objects: [
        this._add('cmdLine', this.cmdLine, {w: 8}),
        this._add('app', this, {removable: false, x: 8}),
        this._add('palette', <ColorPalette />, {x: 8}),
      ],
    }
  }
  ...
  _add = (name, obj, config = {}) => {
    ...
    if (!(typeof obj === 'object' && obj.$$typeof === REACT_ELEMENT_TYPE)) {
      obj = <Repr obj={obj} />
    }

    const gridData = {x: 0, y: Infinity, w: 4, h: 3, minW: 2, minH: 2}
      ...
      >
        ...
        >
          ...
          {config.removable === false || [
            ...
            >
              <span title='remove'><i className='fa fa-window-close'></i></span>
            </span>,
          ]}
        </div>
  ...
  add = (name, obj, config = {}) => {
    if (name in this._objectsByName) {
      ...
      while (name + num in this._objectsByName) {
        num++
      }
      name = name + num
    }
    ...
    this.setState(prevState => ({
      objects: prevState.objects.concat(this._add(name, obj, config)),
    }))
  }
  ...
  renderRepr() {
    return (
      ...
      >
        <span
          className='link'
          onClick={() => this.add('cmdLine', this.cmdLine)}
        >
        ...
        <span
          className='link'
          onClick={() => this.add('palette', <ColorPalette />)}
        >
          <span className='fa-stack fa-lg'>
            <i className='fa fa-square fa-stack-2x'></i>
