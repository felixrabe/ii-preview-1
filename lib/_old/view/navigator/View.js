...
import {changeToJail, join, sanitize} from '../../store/Path'
...
export default class NavigatorView extends React.Component {
  ...
  async update() {
    ...
    const dirPath = join(dataPath, 'ii_dir')
    const data = await this.context.store.get(dataPath)
    const dir = await this.context.store.get(dirPath)
  ...
  saveData = async (data) => {
    await new Promise(resolve => {
      setTimeout(async () => {
        ...
        await this.context.store.set(path, data)
        data = await this.context.store.get(path)
  ...
  renderNav() {
    ...
    const props = {
      delete: async () => await this.context.store.delete(path),
