...
export default connect(
  (state, ownProps) => {
    let editorState = ownProps.item.editorState
    if (!(editorState instanceof D.EditorState)) {
      editorState = editorStateFromText(ownProps.item.text)
    }
    return {editorState}
  },
  (dispatch, ownProps) => ({
    onChange: editorState => dispatch(
      actions.updateItems({
        [ownProps.item.name]: objAssign(ownProps.item, editorState),
      })
    ),
  }),
)(Text)
