...
const Item = (props) => {
  ...
  return (
    <div className='ii-newschool-draggable' style={style}>
      item {props.n}
    </div>
  )
}
...
export default class NewSchoolView extends React.Component {
  ...
  async componentDidMount() {
    ...
    const layout = await this.context.store.get(this.layoutPath)
  ...
  onLayoutChange = (layout) => {
    if (this._isMounted) {
      this.context.store.set(this.layoutPath, layout)
    }
  ...
  render() {
    const animateNone = 'ii-newschool-animate-none'
    const addClassName = this.state.canAnimate ? '' : ' ' + animateNone
    return (
      <div className={`ii-newschool${addClassName}`}>
        <style>{style}</style>
        <ReactGridLayout
          cols={12}
          draggableCancel='.ii-newschool-not-draggable'
          draggableHandle='.ii-newschool-draggable'
